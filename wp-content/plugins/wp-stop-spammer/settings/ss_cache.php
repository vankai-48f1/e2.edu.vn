...
if ( wp_verify_nonce( $nonce, 'ss_stopspam_update' ) ) {
	if ( array_key_exists( 'ss_stop_clear_cache', $_POST ) ) {
		...
		ss_set_stats( $stats );
		echo '<div class="notice notice-success"><p>' . __( 'Cache Cleared', 'wp-stop-spammer' ) . '</p></div>';
	}
...
<div id="ss-plugin" class="wrap">
	<h1 class="ss_head">Stop Spammers — <?php _e( 'Cache', 'wp-stop-spammer' ); ?></h1>
	<?php
	if ( !empty( $msg ) ) {
		echo $msg;
	} ?>
	...
	<form method="post" action="">
		...
		<label class="keyhead">
			<?php _e( 'Bad IP Cache Size', 'wp-stop-spammer' ); ?>
			<br />
		...
		<br />
		<label class="keyhead">
			<?php _e( 'Good IP Cache Size', 'wp-stop-spammer' ); ?>
			<br />
	...
	if ( count( $badips ) == 0 && count( $goodips ) == 0 ) {
		_e( 'Nothing in the cache.', 'wp-stop-spammer' );
	} else {
		?>
		<h2><?php _e( 'Cached Values', 'wp-stop-spammer' ); ?></h2>
		<form method="post" action="">
			<input type="hidden" name="ss_stop_spammers_control" value="<?php echo $nonce; ?>" />
			<input type="hidden" name="ss_stop_clear_cache" value="true" />
			<p class="submit"><input class="button-primary" value="<?php _e( 'Clear the Cache', 'wp-stop-spammer' ); ?>" type="submit" /></p>
		</form>
		<table>
			<tr>
				<?php
				if ( count( $badips ) > 0 ) {
					arsort( $badips );
					?>
					<td width="30%"><?php _e( 'Bad IPs', 'wp-stop-spammer' ); ?></td>
					<?php
				}
				?>
				<?php
				if ( count( $goodips ) > 0 ) {
					?>
					<td width="30%"><?php _e( 'Good IPs', 'wp-stop-spammer' ); ?></td>
					<?php
				}
				?>
			</tr>
