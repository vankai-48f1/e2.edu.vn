...
<div id="ss-plugin" class="wrap">
	<h1 class="ss_head">Stop Spammers — <?php _e( 'Diagnostics & Threat Scan', 'wp-stop-spammer' ); ?></h1>
	<form method="post" action="">
		<div class="ss_info_box">
			<input type="hidden" name="action" value="update" />
			<input type="hidden" name="ss_stop_spammers_control" value="<?php echo $nonce; ?>" />
			<div class="mainsection"><?php _e( 'Option Testing', 'wp-stop-spammer' ); ?>
			...
			<?php _e( '<p>Run the settings against an IP address to see the results.</p>IP Address:<br />', 'wp-stop-spammer' ); ?>
			<input id="ssinput" name="ip" type="text" value="<?php echo $ip; ?>">
			<?php _e( '(Your server address is', 'wp-stop-spammer' ); ?> <?php echo $hip; ?>)<br /><br />
			<?php _e( 'Email:', 'wp-stop-spammer' ); ?><br />
			<input id="ssinput" name="email" type="text" value="<?php echo $email; ?>" /><br /><br />
			<?php _e( 'Author/User:', 'wp-stop-spammer' ); ?><br />
			<input id="ssinput" name="author" type="text" value="<?php echo $author; ?>" /><br /><br />
			<?php _e( 'Subject:', 'wp-stop-spammer' ); ?><br />
			<input id="ssinput" name="subject" type="text" value="<?php echo $subject; ?>" /><br /><br />
			<?php _e( 'Comment:', 'wp-stop-spammer' ); ?><br />
			<textarea name="body"><?php echo $body; ?></textarea><br />
			<div style="width:50%;float:left">
				<p class="submit"><input name="testopt" class="button-primary" value="<?php _e( 'Test Options', 'wp-stop-spammer' ); ?>" type="submit" /></p>
			</div>
			<div style="width:50%;float:right">
				<p class="submit"><input name="testcountry" class="button-primary" value="<?php _e( 'Test Countries', 'wp-stop-spammer' ); ?>" type="submit" /></p>
			</div>
			<br style="clear:both" />
			<?php
			...
			if ( !empty( $nonce ) && wp_verify_nonce( $nonce, 'ss_stopspam_update' ) ) {
				...
				if ( array_key_exists( 'testopt', $_POST ) ) {
					...
					_e( '<br />Memory Used: ' . $m1 . ' Peak: ' . $m2 . '<br />', 'wp-stop-spammer' );
					_e( '<ul>Allow Checks<br />', 'wp-stop-spammer' );
					...
					$m2 = memory_get_peak_usage( true );
					_e( '<br />Memory Used: ' . $m1 . ' Peak: ' . $m2 . '<br />', 'wp-stop-spammer' );
					_e( '<ul>Block Checks<br />', 'wp-stop-spammer' );
					foreach ( $optionlist as $chk ) {
						$ansa = be_load( $chk, $ip, $stats, $options, $post );
						if ( empty( $ansa ) ) {
							$ansa = 'OK';
		...
		<div class="ss_info_box">
			<div class="mainsection"><?php _e( 'Information Display', 'wp-stop-spammer' ); ?>
			...
			<div style="width:50%;float:left">
				<h2><?php _e( 'Display All Options', 'wp-stop-spammer' ); ?></h2>
				<p><?php _e( 'You can dump all options here (useful for debugging):', 'wp-stop-spammer' ); ?></p>
				<p class="submit"><input name="dumpoptions" class="button-primary" value="<?php _e( 'Dump Options', 'wp-stop-spammer' ); ?>" type="submit" /></p>
			</div>
			<div style="width:50%;float:right">
				<h2><?php _e( 'Display All Stats', 'wp-stop-spammer' ); ?></h2>
				<p><?php _e( 'You can dump all stats here: ', 'wp-stop-spammer' ); ?></p>
				<p class="submit"><input name="dumpstats" class="button-primary" value="<?php _e( 'Dump Stats', 'wp-stop-spammer' ); ?>" type="submit" /></p>
			</div>
			<br style="clear:both" />
			<?php
			if ( array_key_exists( 'ss_stop_spammers_control', $_POST ) ) {
	...
	<div class="ss_info_box">
		<div class="mainsection"><?php _e( 'Debugging', 'wp-stop-spammer' ); ?></div>
		<?php
		...
		if ( file_exists( $dfile ) ) {
			...
			if ( !empty( $nonce ) && wp_verify_nonce( $nonce, 'ss_stopspam_update' ) ) {
				if ( array_key_exists( 'killdebug', $_POST ) ) {
					$f = unlink( $dfile );
					_e( '<p>File Deleted<p>', 'wp-stop-spammer' );
				}
			}
		}
		if ( file_exists( $dfile ) ) {
		...
		if ( !empty( $nonce ) && wp_verify_nonce( $nonce, 'ss_stopspam_update' ) ) {
			if ( array_key_exists( 'showdebug', $_POST ) ) {
				_e( '<p><strong>Debug Output:</strong></p><pre>$f</pre><p><strong>end of file (if empty, there are no errors to display)</p></strong>', 'wp-stop-spammer' );
			}
		}
		$nonce = wp_create_nonce( 'ss_stopspam_update' );
		?>
		<div style="width:50%;float:left">
			<form method="post" action="">
				...
				<input type="hidden" name="ss_stop_spammers_control" value="<?php echo $nonce; ?>" />
				<p class="submit"><input class="button-primary" name="showdebug" value="<?php _e( 'Show Debug File', 'wp-stop-spammer' ); ?>" type="submit" /></p>
			</form>
		</div>
		<div style="width:50%;float:right">
			<form method="post" action="">
				<input type="hidden" name="update_options" value="update" />
				<input type="hidden" name="ss_stop_spammers_control" value="<?php echo $nonce; ?>" />
				<p class="submit"><input class="button-primary" name="killdebug" value="<?php _e( 'Delete Debug File', 'wp-stop-spammer' ); ?>" type="submit" /></p>
			</form>
		</div>
	<br style="clear:both" />
	</div>
	...
	if ( $pinf ) { ?>
		<a href="" onclick="document.getElementById('shpinf').style.display='block';return false;" class="button-primary"><?php _e( 'Show PHP Info', 'wp-stop-spammer' ); ?></a>
		<?php
		ob_start();
		phpinfo();
	...
	?>
	<div class="ss_info_box">
		<div class="mainsection"><?php _e( 'Threat Scan', 'wp-stop-spammer' ); ?>
		...
		<?php _e( '<p>A very simple scan that looks for things out of place in the content directory as well as the database.</p>', 'wp-stop-spammer' ); ?>
		<form method="post" action="">
			<input type="hidden" name="update_options" value="update" />
			<input type="hidden" name="ss_stop_spammers_control" value="<?php echo $nonce; ?>" />
			<p class="submit"><input class="button-primary" value="<?php _e( 'Run Scan', 'wp-stop-spammer' ); ?>" type="submit" /></p>
		</form>
	</div>
	<?php if ( $runscan ) { ?>
		<h2><?php _e( 'A clean scan does not mean you are safe. Please keep regular backups and ensure your installation up-to-date!', 'wp-stop-spammer' ); ?></h2>
		<hr />
		<?php
		$disp = false;
		flush();
		// lets try the posts - looking for script tags in data
		_e( '<br /><br />Testing Posts<br />', 'wp-stop-spammer' );
		$ptab = $pre . 'posts';
		...
		if ( $myrows ) {
			foreach ( $myrows as $myrow ) {
				...
				_e( 'found possible problems in post (' . $reason . ') ID: ', 'wp-stop-spammer' ) . $myrow->ID . '<br />';
			}
		} else {
			_e( '<br />Nothing found in posts.<br />', 'wp-stop-spammer' );
			$disp = false;
		}
		echo '<hr />';
		// comments: comment_ID: author_url, comment_agent, comment_author, comment_email
		$ptab = $pre . 'comments';
		_e( '<br /><br />Testing Comments<br />', 'wp-stop-spammer' );
		flush();
		...
		if ( $myrows ) {
			foreach ( $myrows as $myrow ) {
				...
				_e( 'found possible problems in comment (' . $reason . ') ID', 'wp-stop-spammer' ) . $myrow->comment_ID . '<br />';
			}
		} else {
			_e( '<br />Nothing found in comments.<br />', 'wp-stop-spammer' );
		}
		flush();
		echo '<hr />';
		// links: links_id: link_url, link_image, link_description, link_notes, link_rss,link_rss
		$ptab   = $pre . 'links';
		_e( '<br /><br />Testing Links<br />', 'wp-stop-spammer' );
		flush();
		...
		if ( $myrows ) {
			foreach ( $myrows as $myrow ) {
				...
				_e( 'found possible problems in links (' . $reason . ') ID:', 'wp-stop-spammer' ) . $myrow->link_ID . '<br />';
			}
		} else {
			_e( '<br />Nothing found in links.<br />', 'wp-stop-spammer' );
		}
		echo '<hr />';
		// users: ID: user_login, user_nicename, user_email, user_url, display_name
		$ptab = $pre . 'users';
		_e( '<br /><br />Testing Users<br />', 'wp-stop-spammer' );
		flush();
		...
		if ( $myrows ) {
			foreach ( $myrows as $myrow ) {
				...
				_e( 'found possible problems in Users (' . $reason . ') ID:', 'wp-stop-spammer' ) . $myrow->ID . '<br />';
			}
		} else {
			_e( '<br />Nothing found in users.<br />', 'wp-stop-spammer' );
		}
		echo '<hr />';
		// options: option_id option_value, option_name
		// I may have to update this as new websites show up
		$ptab = $pre . 'options';
		_e( '<br /><br />Testing Options Table for HTML<br />', 'wp-stop-spammer' );
		flush();
		$badguys = array(
			'eval('							     => __( 'eval function found', 'wp-stop-spammer' ),
			'eval ('							 => __( 'eval function found', 'wp-stop-spammer' ),
			'networkads'						 => __( 'unexpected network ads reference', 'wp-stop-spammer' ),
			'document.write(unescape('		     => __( 'javascript document write unescape', 'wp-stop-spammer' ),
			'try{window.onload'				     => __( 'javascript onload event', 'wp-stop-spammer' ),
			'escape(document['				     => __( 'javascript checking document array', 'wp-stop-spammer' ),
			'escape(navigator['				     => __( 'javascript checking navigator', 'wp-stop-spammer' ),
			'document.write(string.fromcharcode' => __( 'obsfucated javascript write', 'wp-stop-spammer' ),
			'(base64' . '_decode'				 => __( 'base64 decode to hide code', 'wp-stop-spammer' ),
			'(gz' . 'inflate'					 => __( 'gzip inflate often used to hide code', 'wp-stop-spammer' ),
			'UA-27917097-1'					     => __( 'Bogus Google Analytics code', 'wp-stop-spammer' ),
			'w.wpquery.o'						 => __( 'Malicious jquery in bootleg plugin or theme', 'wp-stop-spammer' ),
			'<scr\\\'+'						     => __( 'Obfuscated script tag, usually in bootleg plugin or theme', 'wp-stop-spammer' )
		);
		...
		if ( $myrows ) {
			foreach ( $myrows as $myrow ) {
				...
				_e( '<strong>Found possible problems in Option $name (' . $reason . ')</strong> option_id:', 'wp-stop-spammer' )
					 . $myrow->option_id . ', value: $line<br /><br />';
			}
		} else {
			_e( '<br />Nothing found in options.<br />', 'wp-stop-spammer' );
		}
		echo '<hr />';
		_e( '<h2>Scanning Themes and Plugins for eval</h2>', 'wp-stop-spammer' );
		flush();
		if ( ss_scan_for_eval() ) {
			$disp = true;
		}
		if ( $disp ) { ?>
			<h2><?php _e( 'Possible Problems Found!', 'wp-stop-spammer' ); ?></h2>
			...
			<p><?php _e( 'Although there are legitimate reasons for using the eval
				...
				code. If eval() comes up in a scan, try to get rid of it.', 'wp-stop-spammer' ); ?></p>
			<p><?php _e( 'Your code could contain "eval", or "document.write(unescape(" or
				...
				but make sure.', 'wp-stop-spammer' ); ?></p>
		<?php } else { ?>
			<h2><?php _e( 'No Problems Found', 'wp-stop-spammer' ); ?></h2>
			<p><?php _e( 'It appears that there are no eval or suspicious JavaScript
				...
				threat may be well-hidden.', 'wp-stop-spammer' ); ?></p>
		<?php }
		flush();
	} // end if runscan
