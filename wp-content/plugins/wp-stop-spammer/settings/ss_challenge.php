...
if ( wp_verify_nonce( $nonce, 'ss_stopspam_update' ) ) {
	if ( array_key_exists( 'action', $_POST ) ) {
		...
		if ( $chkcaptcha == 'G' && ( $recaptchaapisecret == '' || $recaptchaapisite == '' ) ) {
			$chkcaptcha			   = 'Y';
			$options['chkcaptcha'] = $chkcaptcha;
			$msg				   = __( 'You cannot use Google reCAPTCHA unless you have entered an API key.', 'wp-stop-spammer' );
		}
		if ( $chkcaptcha == 'S' && ( $solvmediaapivchallenge == '' || $solvmediaapiverify == '' ) ) {
			$chkcaptcha			   = 'Y';
			$options['chkcaptcha'] = $chkcaptcha;
			$msg				   = __( 'You cannot use Solve Media CAPTCHA unless you have entered an API key.', 'wp-stop-spammer' );
		}
		ss_set_options( $options );
		extract( $options ); // extract again to get the new options
	}
	$update = '<div class="notice notice-success is-dismissible"><p>' . __( 'Options Updated', 'wp-stop-spammer' ) . '</p></div>';
}
...
<div id="ss-plugin" class="wrap">
	<h1 class="ss_head">Stop Spammers — <?php _e( 'Challenge & Block', 'wp-stop-spammer' ); ?></h1>
	<?php if ( !empty( $update ) ) {
		echo "$update";
	} ?>
	<?php if ( !empty( $msg ) ) {
	...
	<form method="post" action="">
		...
		<br />
		<div class="mainsection"><?php _e( 'Access Blocked Message', 'wp-stop-spammer' ); ?>
		...
		<br />
		<div class="mainsection"><?php _e( 'Routing and Notifications', 'wp-stop-spammer' ); ?>
		...
		<div class="checkbox switcher">
			<label id="ss_subhead" for="redir">
				<input class="ss_toggle" type="checkbox" id="redir" name="redir" value="Y" onclick="ss_show_option()" <?php if ( $redir == 'Y' ) { echo 'checked="checked"'; } ?> /><span><small></small></span>
		  		<small><span style="font-size:16px!important"><?php _e( 'Send Visitor to Another Web Page', 'wp-stop-spammer' ); ?></span></small>
			</label>
		</div>
		<br />
		<span id="ss_show_option" style="margin-left:30px;margin-bottom:15px;display:none"><?php _e( 'Redirect URL:', 'wp-stop-spammer' ); ?>
		...
		<div class="checkbox switcher">
			<label id="ss_subhead" for="wlreq">
				...
				<small><span style="font-size:16px!important"><?php _e( 'Blocked users see the Allow Request form', 'wp-stop-spammer' ); ?></span></small>
			</label>
		</div>
		<br />
		<div class="checkbox switcher">
			<label id="ss_subhead" for="notify">
				<input class="ss_toggle" type="checkbox" id="notify" name="notify" value="Y" onclick="ss_show_notify()" <?php if ( $notify == 'Y' ) { echo 'checked="checked"'; } ?> /><span><small></small></span>
		  		<small><span style="font-size:16px!important;"><?php _e( 'Notify Web Admin when a user requests to be added to the Allow List', 'wp-stop-spammer' ); ?></span></small>
			</label>
		</div>
		<br />
		<span id="ss_show_notify" style="margin-left:30px;margin-bottom:15px;display:none"><?php _e( '(Optional) Specify where email requests are sent:', 'wp-stop-spammer' ); ?>
		...
		<div class="checkbox switcher">
			<label id="ss_subhead" for="emailrequest">
				...
				<small><span style="font-size:16px!important"><?php _e( 'Notify Requester when a Web Admin has approved their request to be added to the Allow List', 'wp-stop-spammer' ); ?></span></small>
			</label>
		</div>
		<br />
		<br />
		<div class="mainsection"><?php _e( 'CAPTCHA', 'wp-stop-spammer' ); ?>
		...
		<div style="margin-left:30px">
			<small><span style="font-size:16px!important;"><?php _e( 'Second Chance CAPTCHA Challenge', 'wp-stop-spammer' ); ?></span></small>
			<?php
			if ( !empty( $msg ) ) {
				echo '<span style="color:red;font-size:1.2em">' . $msg . '</span>';
			}
			...
			<?php _e( 'No CAPTCHA (default)', 'wp-stop-spammer' ); ?><br />
			<input type="radio" value="G" name="chkcaptcha" <?php if ( $chkcaptcha == 'G' ) { echo 'checked="checked"'; } ?> />
			<?php _e( 'Google reCAPTCHA', 'wp-stop-spammer' ); ?><br />
			<input type="radio" value="S" name="chkcaptcha" <?php if ( $chkcaptcha == 'S' ) { echo 'checked="checked"'; } ?> />
			<?php _e( 'Solve Media CAPTCHA', 'wp-stop-spammer' ); ?><br />
			<input type="radio" value="A" name="chkcaptcha" <?php if ( $chkcaptcha == 'A' ) { echo 'checked="checked"'; } ?> />
			<?php _e( 'Arithmetic Question', 'wp-stop-spammer' ); ?>
			<p><?php _e( 'To use either the Solve Media or Google reCAPTCHA, you will need an API key.', 'wp-stop-spammer' ); ?></p>
		</div>
		<br />
		<div style="margin-left:30px">
			<small><span style="font-size:16px!important;"><?php _e( 'Google reCAPTCHA API Key', 'wp-stop-spammer' ); ?></span></small><br />
			<input size="64" name="recaptchaapisite" type="text" placeholder="<?php _e( 'Site Key', 'wp-stop-spammer' ); ?>" value="<?php echo $recaptchaapisite; ?>" />
			<br />
			<input size="64" name="recaptchaapisecret" type="text" placeholder="<?php _e( 'Secret Key', 'wp-stop-spammer' ); ?>" value="<?php echo $recaptchaapisecret; ?>" />
			<br />
			<?php if ( !empty( $recaptchaapisite ) ) { ?>
				<script src="https://www.google.com/recaptcha/api.js" async defer></script>
				<div class="g-recaptcha" data-sitekey="<?php echo $recaptchaapisite; ?>"></div>
				<?php _e( 'If the reCAPTCHA form looks good, you need to enable the reCAPTCHA on the Challenge &amp; Block options page. (see left)', 'wp-stop-spammer' ); ?>
			<?php } ?>
			<br />
			<small><span style="font-size:16px!important"><?php _e( 'Solve Media CAPTCHA API Key', 'wp-stop-spammer' ); ?></span></small><br />
			<input size="64" name="solvmediaapivchallenge" type="text" placeholder="<?php _e( 'Challenge Key', 'wp-stop-spammer' ); ?>" value="<?php echo $solvmediaapivchallenge; ?>" />
			<br />
			<input size="64" name="solvmediaapiverify" type="text" placeholder="<?php _e( 'Verification Key', 'wp-stop-spammer' ); ?>" value="<?php echo $solvmediaapiverify; ?>" />
			<br />
			<?php if ( !empty( $solvmediaapivchallenge ) ) { ?>
				...
				<?php _e( '<p>If the CAPTCHA form looks good, you need to enable the CAPTCHA on the Challenge &amp; Block options page. (see left)</p>', 'wp-stop-spammer' ); ?>
			<?php } ?>
		</div>
		<br />
		<br />
