...
<div id="ss-plugin" class="wrap">
	<h1 class="ss_head">Stop Spammers — <?php _e( 'Web Services', 'wp-stop-spammer' ); ?></h1>
	<?php if ( !empty( $msg ) ) {
		echo $msg;
	} ?>
	<br />
	<div class="ss_info_box">
		<p><?php _e( 'Below are several services that can be enabled to check for spam or protect your website against spammers. To learn more about each service and find links to create keys, please <a href="https://stopspammers.io/documentation/web-services/" target="_blank">review our documentation</a>.', 'wp-stop-spammer' ); ?></p>
	</div>
	<br />
	<form method="post" action="">
		...
		<div class="checkbox switcher">
	  		<label id="ss_subhead" for="chkdnsbl">
				<input class="ss_toggle" type="checkbox" id="chkdnsbl" name="chkdnsbl" value="Y" <?php if ( $chkdnsbl == 'Y' ) { echo 'checked="checked"'; } ?> /><span><small></small></span>
		  		<small><span style="font-size:16px!important;"><?php _e( 'Check Against DNSBL Lists Such as Spamhaus.org', 'wp-stop-spammer' ); ?></span></small>
			</label>
		</div>	  
		<br />		
		<div class="checkbox switcher">
	  		<label id="ss_subhead" for="chksfs">
				<input class="ss_toggle" type="checkbox" id="chksfs" name="chksfs" value="Y" <?php if ( $chksfs == 'Y' ) { echo 'checked="checked"'; } ?> /><span><small></small></span>
		  		<small><span style="font-size:16px!important;"><?php _e( 'Enable Stop Forum Spam Lookups', 'wp-stop-spammer' ); ?></span></small>
			</label>
		</div>
		<br />
		<label class="keyhead">
			<?php _e( 'StopForumSpam.com API Key', 'wp-stop-spammer' ); ?>
			<br />
		...
		<br />
		<label class="keyhead">
			<?php _e( 'Project Honeypot API Key', 'wp-stop-spammer' ); ?>
			<br />
		...
		<table cellspacing="1" style="background-color:#ccc;font-size:0.9em;margin-left:30px">
			<tr bgcolor="white">
				<td valign="top"><?php _e( 'Block spammers found on Project HoneyPot with incidents less than
					...
					5 is fairly low.)', 'wp-stop-spammer' ); ?>
				</td>
			</tr>
		</table>
		<br />
		<br />
		<label class="keyhead">
			<?php _e( 'BotScout API Key', 'wp-stop-spammer' ); ?>
			<br />
		...
		<br />
		<label class="keyhead">
			<?php _e( 'Google Safe Browsing API Key', 'wp-stop-spammer' ); ?>
			<br />
