...
<div id="ss-plugin" class="wrap">
	...
	<?php if ( array_key_exists( 'autol', $_POST ) || array_key_exists( 'delo', $_POST ) ) {
		echo '<div class="notice notice-success is-dismissible"><p>' . __( 'Options Updated', 'wp-stop-spammer' ) . '</p></div>';
	}
	?>
	<div class="ss_info_box">
	<p><?php _e( 'Inspect and delete orphan or suspicious options or change plugin options so that they don&acute;t autoload. Be aware that you can break some plugins by deleting their options. Before making updates, please <a href="https://stopspammers.io/documentation/database-cleanup/" target="_blank">review our documentation</a>.', 'wp-stop-spammer' ); ?></p></div>
	<?php
	global $wpdb;
	$ptab  = $wpdb->options;
	$nonce = '';
	...
	if ( !empty( $nonce ) && wp_verify_nonce( $nonce, 'ss_update' ) ) {
		if ( array_key_exists( 'view', $_POST ) ) {
			...
			$v = print_r( $v, true );
			$v = htmlentities( $v );
			_e( '<h2>contents of ' . $op . '</h2><pre>' . $v . '</pre>', 'wp-stop-spammer' );
		}
		if ( array_key_exists( 'autol', $_POST ) ) {
			...
			foreach ( $autol as $name ) {
				...
				_e( 'changing ' . $name . ' autoload to $au<br />', 'wp-stop-spammer' );
		...
		if ( array_key_exists( 'delo', $_POST ) ) {
			foreach ( $_POST['delo'] as $name ) {
				...
				_e( 'deleting ' . $name . ' <br />', 'wp-stop-spammer' );
	...
	<form method="post" name="DOIT2" action="">
		...
		<table id="sstable" name="sstable" cellspacing="2">
			<thead>
			<tr bgcolor="#fff">
				<th class="ss_cleanup"><?php _e( 'Option', 'wp-stop-spammer' ); ?></th>
				<th class="ss_cleanup"><?php _e( 'Autoload', 'wp-stop-spammer' ); ?></th>
				<th class="ss_cleanup"><?php _e( 'Size', 'wp-stop-spammer' ); ?></th>
				<th class="ss_cleanup"><?php _e( 'Change Autoload', 'wp-stop-spammer' ); ?></th>
				<th class="ss_cleanup"><?php _e( 'Delete', 'wp-stop-spammer' ); ?></th>
				<th class="ss_cleanup"><?php _e( 'View Contents', 'wp-stop-spammer' ); ?></th>
			</tr>
			</thead>
			<?php
			foreach ( $rows as $row ) {
				...
				<tr class="ss_cleanup_tr" bgcolor="#fff">
					...
					<td align="center"><button type="submit" name="view" value="<?php echo $option_name; ?>"><?php _e( 'view', 'wp-stop-spammer' ); ?></button></td>
				</tr>
				<?php
			}
			?>
		</table>
		<p class="submit"><input class="button-primary" value="<?php _e( 'Update', 'wp-stop-spammer' ); ?>" type="submit" onclick="return confirm('Are you sure? These changes are permenant.');"></p>
	</form>
	<?php
	$m1 = memory_get_usage();
	...
	$m1 = number_format( $m1 );
	$m3 = number_format( $m3 );
	_e( '<p>Memory Usage Currently: ' . $m1 . ' Peak: ' . $m3 . '</p>', 'wp-stop-spammer' );
	...
	if ( $showtransients && countTransients() > 0 ) { // personal use - probably too dangerous for casual users ?>
		<hr />
		<p><?php _e( 'WordPress creates temporary objects in the database called transients.<br />WordPress is not good about cleaning them up afterwards. You can clean these up safely and it might speed things up.', 'wp-stop-spammer' ); ?></p>
		<form method="post" name="DOIT2" action="">
			<input type="hidden" name="ss_opt_tdel" value="<?php echo $nonce; ?>" />
			<p class="submit"><input class="button-primary" value="<?php _e( 'Delete Transients', 'wp-stop-spammer' ); ?>" type="submit" /></p>
		</form>
		<?php
		$nonce = '';
		...
		?>
		<p><?php _e( 'Currently there are ' . countTransients() . ' found.', 'wp-stop-spammer' ); ?></p>
		<?php
	}
	?>
</div>
