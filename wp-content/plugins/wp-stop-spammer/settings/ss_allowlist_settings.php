...
if ( !empty( $nonce ) && wp_verify_nonce( $nonce, 'ss_stopspam_update' ) ) {
	...
	$msg = __( '<div class="notice notice-success is-dismissible"><p>Options Updated</p></div>', 'wp-stop-spammer' );
}
...
<div id="ss-plugin" class="wrap">
	<h1 class="ss_head"><?php _e( 'Stop Spammers — Allow Requests & Lists', 'wp-stop-spammer' ); ?></h1>
	<?php if ( !empty( $msg ) ) {
		echo $msg;
	} ?>
	<br />
	<br />
	<div class="mainsection"><?php _e( 'Allow List Requests', 'wp-stop-spammer' ); ?>
		<sup class="ss_sup"><a href="https://stopspammers.io/documentation/allow-requests-and-lists/#allowrequests" target="_blank"><i class="fa fa-question-circle fa-2x tooltip"></i></a></sup>
	</div>
	<?php
	if ( count( $wlrequests ) == 0 ) {
		_e( '<p><strong>There are currently no pending requests.</strong></p>', 'wp-stop-spammer' );
	} else { ?>
	<form method="post" action="">
		<input type="hidden" name="ss_stop_spammers_control" value="<?php echo $nonce; ?>" />
		<input type="hidden" name="ss_stop_clear_wlreq" value="true" />
		<p class="submit"><input class="button-primary" value="<?php _e( 'Clear the Requests', 'wp-stop-spammer' ); ?>" type="submit" /></p>
	</form>
	<table id="sstable" name="sstable" cellspacing="2">
		<thead>
			<tr style="background-color:#675682;color:white;text-align:center;text-transform:uppercase;font-weight:600">
				<th><?php _e( 'Time', 'wp-stop-spammer' ); ?></th>
				<th><?php _e( 'IP', 'wp-stop-spammer' ); ?></th>
				<th><?php _e( 'Email', 'wp-stop-spammer' ); ?></th>
				<th><?php _e( 'Reason', 'wp-stop-spammer' ); ?></th>
				<th><?php _e( 'Message', 'wp-stop-spammer' ); ?></th>
			</tr>
		</thead>
		<tbody id="wlreq">
			<?php
	...
	<form method="post" action="">
		...
		<input type="hidden" name="ss_stop_spammers_control" value="<?php echo $nonce; ?>" />
		<div class="mainsection"><?php _e( 'Personalized Allow List', 'wp-stop-spammer' ); ?>
		...
		<div class="checkbox switcher">
	  		<label id="ss_subhead" for="chkwluserid">
				<input class="ss_toggle" type="checkbox" id="chkwluserid" name="chkwluserid" value="Y" <?php if ( $chkwluserid == 'Y' ) { echo 'checked="checked"'; } ?> /><span><small></small></span>
		  		<small><span style="font-size:16px!important"><?php _e( 'Enable Allow by Username (not recommended)', 'wp-stop-spammer' ); ?></span></small>
			</label>
		</div>
		<br />
		...
		?></textarea>
		<br />
		<div class="mainsection"><?php _e( 'Allow Options', 'wp-stop-spammer' ); ?>
			<sup class="ss_sup"><a href="https://stopspammers.io/documentation/allow-requests-and-lists/#allowoptions" target="_blank"><i class="fa fa-question-circle fa-2x tooltip"></i></a></sup>
		</div>
		<br />
		<div class="checkbox switcher">
			<label id="ss_subhead" for="chkgoogle">
				...
				<small><span style="font-size:16px!important"><?php _e( 'Google (keep enabled under most circumstances)', 'wp-stop-spammer' ); ?></span></small>
			</label>
		</div>
		<br />
		<div class="checkbox switcher">
	  		<label id="ss_subhead" for="chkgenallowlist">
				...
				<small><span style="font-size:16px!important"><?php _e( 'Generated Allow List', 'wp-stop-spammer' ); ?></span></small>
			</label>
		</div>
		<br />
		<div class="checkbox switcher">
	  		<label id="ss_subhead" for="chkmiscallowlist">
				...
				<small><span style="font-size:16px!important"><?php _e( 'Other Allow Lists', 'wp-stop-spammer' ); ?></span></small>
			</label>
		</div>
		<br />
		<div class="checkbox switcher">
	  		<label id="ss_subhead" for="chkpaypal">
				<input class="ss_toggle" type="checkbox" id="chkpaypal" name="chkpaypal" value="Y" <?php if ( $chkpaypal == 'Y' ) { echo 'checked="checked"'; } ?> /><span><small></small></span>
		  		<small><span style="font-size:16px!important"><?php _e( 'Allow', 'wp-stop-spammer' ); ?> PayPal</span></small>
			</label>
		</div>
		<br />
		<div class="checkbox switcher">
	  		<label id="ss_subhead" for="chkstripe">
				...
				<small><span style="font-size:16px!important"><?php _e( 'Allow', 'wp-stop-spammer' ); ?> Stripe</span></small>
			</label>
		</div>
		<br />
		<div class="checkbox switcher">
	  		<label id="ss_subhead" for="chkauthorizenet">
				...
				<small><span style="font-size:16px!important"><?php _e( 'Allow', 'wp-stop-spammer' ); ?> Authorize.Net</span></small>
			</label>
		</div>
		<br />
		<div class="checkbox switcher">
	  		<label id="ss_subhead" for="chkbraintree">
				...
				<small><span style="font-size:16px!important"><?php _e( 'Allow', 'wp-stop-spammer' ); ?> Braintree</span></small>
			</label>
		</div>
		<br />
		<div class="checkbox switcher">
	  		<label id="ss_subhead" for="chkrecurly">
				...
				<small><span style="font-size:16px!important"><?php _e( 'Allow', 'wp-stop-spammer' ); ?> Recurly</span></small>
			</label>
		</div>
		<br />
		<div class="checkbox switcher">
	  		<label id="ss_subhead" for="chkyahoomerchant">
				...
				<small><span style="font-size:16px!important"><?php _e( 'Allow', 'wp-stop-spammer' ); ?> Yahoo Merchant Services</span></small>
			</label>
		</div>
		<br />
		<div class="checkbox switcher">
			<label id="ss_subhead" for="chkaws">
				...
				<small><span style="font-size:16px!important"><?php _e( 'Allow', 'wp-stop-spammer' ); ?> Amazon Cloud</span></small>
			</label>
		</div>
		<br />
