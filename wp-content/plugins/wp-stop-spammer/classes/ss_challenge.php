...
class ss_challenge extends be_module {
	public function process( $ip, &$stats = array(), &$options = array(), &$post = array() ) {
		...
		*/
		...
		if ( !empty( $_POST ) && array_key_exists( 'kn', $_POST ) ) {
			...
			if ( !empty( $nonce ) && wp_verify_nonce( $nonce, 'ss_stopspam_block' ) ) {
				...
				$msg = __( 'Thank you,<br />', 'wp-stop-spammer' );
				if ( $emailsent ) {
					$msg .= __( 'The webmaster has been notified by email.<br />', 'wp-stop-spammer' );
				}
				if ( $allowset ) {
					$msg .= __( 'Your request has been recorded.<br />', 'wp-stop-spammer' );
				}
					...
					case 'G':
						if ( array_key_exists( 'recaptcha', $_POST )
							...
							if ( empty( $recaptchaapisecret ) || empty( $recaptchaapisite ) ) {
								$msg = __( 'reCAPTCHA keys are not set.', 'wp-stop-spammer' );
							} else {
								...
								if ( strpos( $resp, '"success": true' ) !== false ) { // found success
									...
									ss_log_good( $ip, __( 'Passed reCAPTCHA', 'wp-stop-spammer' ), 'pass' );
									do_action( 'ss_stop_spam_ok', $ip, $post ); // so plugins can undo spam report
									return false;
								} else {
									$msg = __( 'Google reCAPTCHA entry does not match. Try again.', 'wp-stop-spammer' );
								}
							}
						}
						break;
					case 'S':
						if ( array_key_exists( 'adcopy_challenge', $_POST ) && !empty( $_POST['adcopy_challenge'] ) ) {
							...
							if ( strpos( $result, 'true' ) !== false ) {
								...
								// sfs_debug_msg( "trying to return the post to the comments program" . print_r( $_POST, true ) );
								// success add to cache
								ss_log_good( $ip, __( 'Passed Solve Media CAPTCHA', 'wp-stop-spammer' ), 'pass' );
								do_action( 'ss_stop_spam_ok', $ip,
									$post ); // so plugins can undo spam report
								return false;
							} else {
								$msg = __( 'CAPTCHA entry does not match. Try again.', 'wp-stop-spammer' );
							}
						}
						break;
					case 'A':
					case 'Y':
						if ( array_key_exists( 'nums', $_POST ) && !empty( $_POST['nums'] ) ) {
							...
							if ( $sum == $nums ) {
								...
								// sfs_debug_msg( "trying to return the post to the comments program" . print_r( $_POST, true ) );
								// success add to cache
								ss_log_good( $ip, __( 'Passed Simple Arithmetic CAPTCHA', 'wp-stop-spammer' ), 'pass' );
								do_action( 'ss_stop_spam_ok', $ip, $post ); // so plugins can undo spam report
								return false;
							} else {
								$msg = __( 'Incorrect. Try again.', 'wp-stop-spammer' );
							}
						}
						break;
					case 'F':
		...
		$captop = __( '<h1>Are you human?</h1>', 'wp-stop-spammer' );
		$capbot = "";
		// now the CAPTCHAs
		$cap = '';
		switch ( $chkcaptcha ) {
	...
	public function ss_send_email( $options = array() ) {
		...
		if ( array_key_exists( 'ke', $_POST ) && !empty( $_POST['ke'] ) ) {
			...
			$subject = __( 'Allow List Request from ', 'wp-stop-spammer' ) . get_bloginfo( 'name' );
			$ip	     = ss_get_ip();
			$web 	 = __( 'Approve or Block Request: ', 'wp-stop-spammer' ) . admin_url( 'admin.php?page=ss_allow_list' );
			$message = __( "
			...
			$headers = __( 'From: ', 'wp-stop-spammer' ) . get_option( 'admin_email' ) . "\r\n";
			wp_mail( $to, $subject, $message, $headers );
			$rejectmessage = __( '<h2>Email sent. Thank you.</h2>', 'wp-stop-spammer' );
			return true;
		}
	}
