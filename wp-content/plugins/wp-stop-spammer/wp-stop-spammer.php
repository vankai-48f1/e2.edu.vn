...
Text Domain: wp-stop-spammer
*/

// networking requires a couple of globals
define( 'SS_VERSION', '2021.8' );
...
function ss_load_plugin_textdomain() {
	load_plugin_textdomain( 'wp-stop-spammer', FALSE, basename( dirname( __FILE__ ) ) . '/languages/' );
}
	...
	foreach ( ( array ) $wp_filter_notices as $filters ) {
		foreach ( $filters as $callback => $callback_array ) {
			...
			if ( $options['ss_keep_hidden_btn'] === 'Y' ) {
				$hide_for_user = "<a data-target='user' data-notice-id='{$uniq_id1}_{$uniq_id2}' class='ss-hide-notice'>" . __( 'Keep Hidden', 'wp-stop-spammer' ) . "</a>";
			}
			if ( $options['ss_hide_all_btn'] === 'Y' ) {
				$hide_for_all = "<a data-target='all' data-notice-id='{$uniq_id1}_{$uniq_id2}' class='ss-hide-notice' href='admin.php?page=ss_options#notificationcontrol'>" . __( 'Hide All Notices', 'wp-stop-spammer' ) . "</a>";
			}
...
function ss_wc_admin_notice() {
	if ( is_plugin_active( 'woocommerce/woocommerce.php' ) ) {
		...
		if ( !get_user_meta( $user_id, 'ss_wc_notice_dismissed' ) && current_user_can( 'manage_options' ) ) {
			echo '<div class="notice notice-info"><p style="color:purple">' . __( '<big><strong>WooCommerce Detected</strong></big> | We recommend keeping <a href="admin.php?page=ss_options">this option</a> enabled to avoid blocking customers.', 'stop-spammers' ) . '<a href="?sswc-dismiss" class="alignright">' . __( 'Dismiss', 'wp-stop-spammer' ) . '</a></p></div>';
		}
	}
}
...
function ss_log_akismet() {
	...
	// not on Allow Lists
	$post		    = get_post_variables();
	$post['reason'] = __( 'from Akismet', 'wp-stop-spammer' );
	$post['chk']	= 'chkakismet';
...
function be_load( $file, $ip, &$stats = array(), &$options = array(), &$post = array() ) {
	...
	if ( is_array( $file ) ) { // add-ons pass their array
		...
		if ( !file_exists( $file[0] ) ) {
			sfs_debug_msg( __( 'not found ', 'wp-stop-spammer' ) . print_r( $add, true ) );
			return false;
		}
		// require_once( $file[0] );
	...
	if ( !file_exists( $fd ) ) {
		_e( '<br /><br />Missing ' . $file, $fd . '<br /><br />', 'wp-stop-spammer' );
		return false;
	}
	require_once( $fd );
	// this loads a be_module class
...
class SSRegDate {
	...
	public static function users_columns( $columns ) {
		$columns['registerdate'] = _x( 'Registered', 'user', 'wp-stop-spammer' );
		return $columns;
	}
...
function ss_user_reg_filter( $user_login ) {
	...
	if ( $reason !== false ) {
		...
		$post['reason'] = __( 'Failed Registration: Bad Cache', 'wp-stop-spammer' );
		$host['chk']	= 'chkbcache';
		$ansa		    = be_load( 'ss_log_bad', ss_get_ip(), $stats, $options, $post );
		wp_die( '$rejectmessage', __( 'Login Access Blocked', 'wp-stop-spammer' ), array( 'response' => 403 ) );
		exit();
	}
	// check periods
	$reason = be_load( 'chkperiods', ss_get_ip(), $stats, $options, $post );
	if ( $reason !== false ) { 
		wp_die( 'Registration Access Blocked', __( 'Login Access Blocked', 'wp-stop-spammer' ), array( 'response' => 403 ) );
	}
	// check the whitelist
	$reason = ss_check_white();
	sfs_errorsonoff();
	if ( $reason !== false ) {
		$post['reason'] = __( 'Passed Registration:', 'wp-stop-spammer' ) . $reason;
	...
	$ret			= be_load( 'ss_check_post', ss_get_ip(), $stats, $options, $post );
	$post['reason'] = __( 'Passed Registration ', 'wp-stop-spammer' ) . $ret;
	$ansa		    = be_load( 'ss_log_good', ss_get_ip(), $stats, $options, $post );
	return $user_login;
}
...
function ss_summary_link( $links ) {
	$links = array_merge( array( '<a href="' . admin_url( 'admin.php?page=stop_spammers' ) . '">' . __( 'Settings', 'wp-stop-spammer' ) . '</a>' ), $links );
	return $links;
}
...
function ss_check_for_premium() {
	if ( !is_plugin_active( 'stop-spammers-premium/stop-spammers-premium.php' ) ) {
		...
		function ss_upgrade_link( $links ) {
			$links = array_merge( array( '<a href="https://stopspammers.io/" title="' . esc_attr__( 'Get Maximum Dynamic Security', 'wp-stop-spammer' ) . '" target="_blank" style="font-weight:bold">' . __( 'Upgrade', 'wp-stop-spammer' ) . '</a>' ), $links );
			return $links;
		}
	}
}
