...
function ss_read_filex( $filename ) {
	...
	return __( 'File Not Found', 'wp-stop-spammer' );
}
...
function sfs_ErrorHandler( $errno, $errmsg, $filename, $linenum, $vars ) {
	...
	switch ( $errno ) {
		case E_ERROR:
			$serrno = __( 'Fatal run-time errors. These indicate errors that can not be recovered from, such as a memory allocation problem. Execution of the script is halted. ', 'wp-stop-spammer' );
			break;
		case E_WARNING:
			$serrno = __( 'Run-time warnings (non-fatal errors). Execution of the script is not halted. ', 'wp-stop-spammer' );
			break;
		case E_NOTICE:
			$serrno = __( 'Run-time notices. Indicate that the script encountered something that could indicate an error, but could also happen in the normal course of running a script. ', 'wp-stop-spammer' );
			break;
		default;
			$serrno = __( 'Unknown Error Type ' . $errno . '', 'wp-stop-spammer' );
	}
	if ( strpos( $errmsg, __( 'modify header information', 'wp-stop-spammer' ) ) ) {
		return false;
	}
	$now = date( 'Y/m/d H:i:s', time() + ( get_option( 'gmt_offset' ) * 3600 ) );
