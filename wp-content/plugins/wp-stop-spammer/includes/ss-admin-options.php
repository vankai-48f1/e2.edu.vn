...
function ss_sp_plugin_action_links( $links, $file ) {
	...
	if ( SS_MU == 'Y' ) {
		$link = '<a href="' . admin_url( 'network/admin.php?page=stop_spammers' ) . '">' . __( 'Settings', 'wp-stop-spammer' ) . '</a>';
	} else {
		$link = '<a href="' . admin_url( 'admin.php?page=stop_spammers' ) . '">' . __( 'Settings', 'wp-stop-spammer' ) . '</a>';
	}
...
function ss_sp_rightnow() {
	...
	if ( $spmcount > 0 ) {
		...
		_e( '<p>Stop Spammers has prevented <strong>' . $spmcount . '</strong> spammers from registering or leaving comments.</p>', 'wp-stop-spammer' );
	}
	if ( count( $wlrequests ) == 1 ) {
		echo '<p><strong>' . count( $wlrequests ) . '</strong> ' . __( 'user has been blocked and <a href="admin.php?page=ss_allow_list">requested</a> that you add them to the Allow List.</p>', 'wp-stop-spammer' );
	} else if ( count( $wlrequests ) > 0 ) {
		echo '<p><strong>' . count( $wlrequests ) . '</strong> ' . __( 'users have been blocked and <a href="admin.php?page=ss_allow_list">requested</a> that you add them to the Allow List.</p>', 'wp-stop-spammer' );
	}
}
...
function ss_row( $actions, $comment ) {
	...
	$action   = "";
	// $action .= "|";
	// $action .= "<a title=\"" . esc_attr__( 'Check Project HoneyPot', 'wp-stop-spammer' ) . "\" target=\"_stopspam\" href=\"https://www.projecthoneypot.org/search_ip.php?ip=$ip\">Check HoneyPot</a>";
	// add the network check
	$whois	  = SS_PLUGIN_URL . 'images/whois.png';
	$who	  = "<a title=\"" . esc_attr__( 'Look Up WHOIS', 'wp-stop-spammer' ) . "\" target=\"_stopspam\" href=\"https://whois.domaintools.com/$ip\"><img src=\"$whois\" height=\"16px\" /></a>";
	$stophand = SS_PLUGIN_URL . 'images/stop.png';
	$stop	  = "<a title=\"" . esc_attr__( 'Check Stop Forum Spam (SFS)', 'wp-stop-spammer' ) . "\" target=\"_stopspam\" href=\"https://www.stopforumspam.com/search.php?q=$ip\"><img src=\"$stophand\" height=\"16px\" /> </a>";
	$action  .= " $who $stop";
	...
	if ( !empty( $email ) ) {
		$action .= "|";
		$action .= "<a $exst title=\"" . esc_attr__( 'Report to Stop Forum Spam (SFS)', 'wp-stop-spammer' ) . "\" $target $href $onclick class='delete:the-comment-list:comment-$ID::delete=1 delete vim-d vim-destructive'>" . __( ' Report to SFS', 'wp-stop-spammer' ) . "</a>";
	}
	$actions['check_spam'] = $action;
	return $actions;
}
...
function sfs_handle_ajax_sub( $data ) {
	...
	if ( empty( $options ) ) { // can't happen?
		_e( ' No Options Set', 'wp-stop-spammer' );
		exit();
	}
	// print_r( $options );
	extract( $options );
	...
	if ( empty( $comment_id ) ) {
		_e( ' No Comment ID Found', 'wp-stop-spammer' );
		exit();
	}
	...
	if ( $comment_id == 'registration' ) {
		...
		if ( empty( $comment ) ) {
			_e( ' No Comment Found for ' . $comment_id . '', 'wp-stop-spammer' );
			exit();
		}
	}
	// print_r( $comment );
	...
	if ( empty( $apikey ) ) {
		_e( 'Cannot Report Spam without API Key', 'wp-stop-spammer' );
		exit();
	}
	...
	if ( stripos( $ret, __( 'data submitted successfully', 'wp-stop-spammer' ) ) !== false ) {
		echo $ret;
	} else if ( stripos( $ret, __( 'recent duplicate entry', 'wp-stop-spammer' ) ) !== false ) {
		_e( ' Recent Duplicate Entry ', 'wp-stop-spammer' );
	} else {
		_e( ' Returning from AJAX: ', 'wp-stop-spammer' ) . $hget . ' - ' . $ret;
	}
	exit();
}
...
function sfs_handle_ajax_check( $data ) {
	if ( !ipChkk() ) {
		_e( ' Not Enabled', 'wp-stop-spammer' );
		exit();
	}
	...
	if ( !empty( $check ) ) {
		...
		if ( $n === false ) {
			...
			_e( ' Access to the Stop Forum Spam Database is Working', 'wp-stop-spammer' );
		}
	} else {
...
function sfs_handle_ajax_sfs_process_watch( $data ) {
	...
	if ( !array_key_exists( 'func', $_GET ) ) {
		_e( ' Function Not Found', 'wp-stop-spammer' );
		exit();
	}
...
// if it is not good or bad IP we don't need the container as it is the log
		...
		case 'delete_wlem': // this is from the Allow Requests list
			...
			break;
		default:
			_e( '\r\n\r\nUnrecognized function "' . $func . '"', 'wp-stop-spammer' );
			exit();
	}
	$ajaxurl  = admin_url( 'admin-ajax.php' );
	...
	switch ( $container ) {
		...
		default:
			...
			_e( 'Something is missing ' . $container . ' ', 'wp-stop-spammer' );
			exit();
	}
}
...
function ss_sfs_ip_column( $value, $column_name, $user_id ) {
	...
	if ( $column_name == 'signup_ip' ) {
		...
		if ( !empty( $signup_ip ) ) {
			...
			$stopper	 = "<a title=\"" . esc_attr__( 'Check Stop Forum Spam (SFS)', 'wp-stop-spammer' ) . "\" target=\"_stopspam\" href=\"https://www.stopforumspam.com/search.php?q=$signup_ip\"><img src=\"$stophand\" height=\"16px\" /></a>";
			$honeysearch = "<a title=\"" . esc_attr__( 'Check Project HoneyPot', 'wp-stop-spammer' ) . "\" target=\"_stopspam\" href=\"https://www.projecthoneypot.org/ip_$signup_ip\"><img src=\"$search\" height=\"16px\" /></a>";
			$botsearch   = "<a title=\"" . esc_attr__( 'Check BotScout', 'wp-stop-spammer' ) . "\" target=\"_stopspam\" href=\"https://botscout.com/search.htm?stype=q&sterm=$signup_ip\"><img src=\"$search\" height=\"16px\" /></a>";
			$who		 = "<br /><a title=\"" . esc_attr__( 'Look Up WHOIS', 'wp-stop-spammer' ) . "\" target=\"_stopspam\" href=\"https://whois.domaintools.com/$signup_ip\"><img src=\"$whois\" height=\"16px\" /></a>";
			$action	     = " $who $stopper $honeysearch $botsearch";
			$options	 = ss_get_options();
			$apikey	     = $options['apikey'];
			if ( !empty( $apikey ) ) {
				$report  = "<a title=\"" . esc_attr__( 'Report to SFS', 'wp-stop-spammer' ) . "\" target=\"_stopspam\" href=\"https://www.stopforumspam.com/add.php?username=$username&email=$useremail&ip_addr=$signup_ip&evidence=$userurl&api_key=$apikey\"><img src=\"$stophand\" height=\"16px\" /></a>";
				$action .= $report;
			}
