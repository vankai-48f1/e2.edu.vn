...
class chkvalidip {
	...
	public function process( $ip, &$stats = array(), &$options = array(), &$post = array() ) {
		if ( empty( $ip ) ) {
			return __( 'Invalid IP: ', 'wp-stop-spammer' ) . $ip;
		}
		...
		if ( $ip == '127.0.0.1' ) {
			return __( 'Accessing Site Through localhost', 'wp-stop-spammer' );
		}
		$priv = array(
		...
		foreach ( $priv as $ips ) {
			if ( $ip2 >= $ips[0] && $ip2 <= $ips[1] ) {
				return __( 'Local IP Address: ', 'wp-stop-spammer' ) . $ip;
			}
			if ( $ip2 < $ips[1] ) {
				break;
		...
		*/
		...
		if ( array_key_exists( 'SERVER_ADDR', $_SERVER ) ) {
			...
			if ( $ip == $lip ) {
				return __( 'IP Same as Server: ', 'wp-stop-spammer' ) . $ip;
			}
		...
		if ( substr( $ip, 0, $j ) == substr( $lip, 0, $k ) ) {
			return __( 'IP same /24 subnet as server ', 'wp-stop-spammer' ) . $ip;
		}
		return false;
	}
}
