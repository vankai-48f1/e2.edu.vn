...
// include Checkviengviet function vn_num_acc_char()
require_once get_template_directory() . '/inc/checktiengviet.php';

function e2post_is_vi()
{
    $title = get_the_title();
    $excerpt = get_the_excerpt();

    if ($title || $excerpt) {
        // kiểm tra title và excerpt có dấu hay không
        $title_vi   = vn_num_acc_char($title);
        $excerpt_vi = vn_num_acc_char($excerpt);

        // var_dump($title_vi || $excerpt_vi);
        if ($title_vi || $excerpt_vi) {
            // kí tự có dấu
            return true;
        }
